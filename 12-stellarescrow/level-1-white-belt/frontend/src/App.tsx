...
const pages = [
  ...
  { id: 'send', label: 'Lock Funds' },
  { id: 'activity', label: 'Escrow Ledger' },
] as const;

const checklist = [
  { title: 'Trustless Escrow Agreements', desc: 'Secure funds locking via Freighter cryptographic authorization.' },
  { title: 'Horizon Ledger Sync', desc: 'Dynamic balance check and escrow vault status updates.' },
  { title: 'Immutable Escrow Memo', desc: 'Commit contract details directly into transaction memo payloads.' },
  { title: 'Arbitration Gating', desc: 'Decentralized resolution paths for escrow release approval.' },
];
...
export default function App() {
  ...
  const [message, setMessage] = useState('StellarEscrow initialization console ready.');
  ...
  async function initiateEscrow() {
    ...
    try {
      ...
      setTxHash(hash);
      setState('success');
      setMessage('Escrow funds successfully locked on-chain!');
      setBalance(await fetchNativeBalance(publicKey));
      setPage('activity');
    } catch (error: any) {
      setState('failure');
      setMessage(error.message ?? 'Escrow schedule lock rejected.');
      setPage('activity');
    }
  }
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="md:col-span-3 luxury-card p-10 rounded-3xl flex flex-col justify-center gap-6">
                  ...
                  <h2 className="text-4xl font-bold tracking-tight text-white leading-tight">
                    Secure Escrow Vault Settlements
                  </h2>
                  <p className="text-stone-300 leading-relaxed text-base">
                    Welcome to StellarEscrow. Lock and manage secure transaction escrows with trustless smart vaults. Connect Freighter to allocate payments, define targets, and settle escrow gates.
                  </p>
                ...
                <div className="md:col-span-2 luxury-card p-8 rounded-3xl flex flex-col justify-between gap-6">
                  <h3 className="font-bold text-xl text-amber-500 border-b border-amber-600/10 pb-4">Escrow Specifications</h3>
                  ...
                  <div className="bg-stone-950/40 border border-stone-800 p-6 rounded-2xl flex flex-col gap-4 font-mono text-xs text-stone-300">
                    <div className="flex justify-between items-center border-b border-stone-900 pb-3">
                      ...
                      <span className={`text-[9px] font-bold px-2.5 py-1 border rounded-xl uppercase tracking-widest ${
                        publicKey ? 'bg-amber-900/10 text-amber-400 border-amber-600/30' : 'bg-rose-950/20 text-rose-400 border-rose-500/30'
                      }`}>
            ...
            {page === 'send' && (
              ...
              >
                <div className="luxury-card p-10 rounded-3xl flex flex-col gap-6">
                  <div className="text-center flex flex-col gap-2">
                    <h2 className="text-3xl font-bold text-amber-500">Lock Escrow Funds</h2>
                    <p className="text-xs text-stone-400">Submit secure payment with lock parameters.</p>
                  </div>

                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-stone-300 uppercase tracking-wider">Beneficiary / Arbitrator Address</label>
                      <input 
                        value={destination} 
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-stone-300 uppercase tracking-wider">Lock Amount (XLM)</label>
                      <input 
                        type="number"
                        value={amount} 
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="text-xs font-bold text-stone-300 uppercase tracking-wider">Arbitration Memo</label>
                      <input 
                        value={memo} 
                  ...
                  >
                    {state === 'submitting' ? 'locking vault...' : 'LOCK ESCROW FUNDS'}
                  </button>
                </div>
              </motion.div>
            )}
            ...
            {page === 'activity' && (
              ...
              >
                <div className="luxury-card p-10 rounded-3xl flex flex-col gap-6">
                  <h2 className="text-3xl font-bold text-center text-amber-500">Escrow Ledger Logs</h2>
