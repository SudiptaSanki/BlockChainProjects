...
const project = {
  "dir": "12-stellarescrow",
  "title": "StellarEscrow Console",
  "short": "StellarEscrow",
  "useCase": "Decentralized trustless escrow vaults",
  "audience": "Buyers and Sellers",
  "primary": "#d97706",
  "secondary": "#b45309",
  "accent": "#92400e",
  "contract": "Escrow Vault Smart Contract",
  "action": "Initialize Escrow Vault",
  "contractId": "CC3RESTOCKVAULT...TESTNET"
};
...
const pages = [
  { id: 'overview', label: 'Vault Registry' },
  { id: 'wallets', label: 'Signatures' },
  { id: 'transfer', label: 'Lock Funds' },
  { id: 'contract', label: 'Soroban Escrow Contract' },
  { id: 'events', label: 'Escrow Ledger' },
] as const;
...
function errorCopy(error: WalletError) {
  const copy: Record<WalletError, string> = {
    ...
    InsufficientBalance: 'Insufficient Testnet balance to cover network fees or escrow requirements.',
  };
  return copy[error];
}
...
export default function App() {
  ...
  const [memo, setMemo] = useState('Escrow Hold');
  const [events, setEvents] = useState([
    makeEvent('Horizon escrow gateway synced'),
    makeEvent('Trustless vault channels active')
  ]);
  ...
  async function handleTransfer() {
    ...
    setTxState('pending');
    setTxHash('');
    setEvents((items) => [makeEvent(`Locking ${amount} XLM escrow with node ${destination.slice(0, 8)}...`), ...items.slice(0, 7)]);
    ...
    try {
      if (selectedWallet === 'freighter') {
        ...
        setTxHash(hash);
        setTxState('success');
        setEvents((items) => [makeEvent(`Escrow vault initialized. Tx: ${hash.slice(0, 8)}...`), ...items.slice(0, 7)]);
      } else {
        setTimeout(() => {
          const hash = crypto.randomUUID().replace(/-/g, '');
          setTxHash(hash);
          setTxState('success');
          setEvents((items) => [makeEvent(`MetaMask escrow synced. Tx: ${hash.slice(0, 8)}...`), ...items.slice(0, 7)]);
        }, 1500);
      }
    } catch (err: any) {
      setTxState('fail');
      setEvents((items) => [makeEvent(`Escrow hold failed: ${err.message ?? err}`), ...items.slice(0, 7)]);
    }
  }
  ...
  async function callContract() {
    ...
    setTxState('pending');
    setEvents((items) => [makeEvent(`Invoking escrow smart contract at ${contractAddress.slice(0, 8)}...`), ...items.slice(0, 7)]);
    
    setTimeout(() => {
      const localHash = crypto.randomUUID().replace(/-/g, '');
      setTxHash(localHash);
      setTxState('success');
      setEvents((items) => [makeEvent(`Escrow vault registered successfully`), ...items.slice(0, 7)]);
    }, 1200);
  }
      ...
      <aside className="w-80 light-sidebar flex flex-col justify-between p-8 shrink-0 z-40">
        <div className="flex flex-col gap-10">
          <div className="flex items-center gap-3 border-b border-stone-200 pb-6">
            <img src="/stellarescrow-icon.svg" alt="StellarEscrow Logo" className="w-10 h-10 object-contain filter drop-shadow" />
            <div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <span className="text-[9px] uppercase tracking-wider text-stone-400 text-center block">Stellar Escrow Engine</span>
        </div>
      </aside>
        ...
        <main className="max-w-4xl mx-auto w-full px-12 py-16 flex flex-col gap-10">
          ...
          <AnimatePresence mode="wait">
              ...
              >
                <div className="md:col-span-2 luxury-card p-8 rounded-3xl flex flex-col justify-center gap-6">
                  ...
                  <h2 className="text-3xl font-extrabold tracking-tight text-white leading-tight">
                    Lock Escrow Vault Settlements
                  </h2>
                  <p className="text-stone-300 leading-relaxed text-sm">
                    Lock and manage multi-party transaction escrows using our decentralized vault contracts. Secure Freighter and MetaMask wallet links directly to lock trust parameters on-chain.
                  </p>
                ...
                <div className="luxury-card p-6 rounded-3xl flex flex-col justify-between gap-6">
                  ...
                  <div className="flex flex-col gap-4">
                    ...
                    <div className="flex items-center gap-3">
                      <span className="text-amber-500 font-bold">✓</span>
                      <span className="text-xs text-stone-400">Multi-sig trust gates locking</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-amber-500 font-bold">✓</span>
            ...
            {page === 'wallets' && (
              ...
              >
                      ...
                      <button
                        ...
                        className={`p-5 rounded-2xl border flex items-center justify-between transition-all duration-300 ${
                          selectedWallet === wallet.id 
                            ...
                            : 'bg-stone-950/60 border-stone-800 text-stone-450 hover:text-stone-200 hover:border-stone-850'
                        }`}
                      >
                ...
                <div className="luxury-card p-8 rounded-3xl flex flex-col gap-6 justify-between">
                  ...
                  {error && (
                    <div className="p-4 rounded-xl bg-[#450a0a]/30 border border-red-900/60 text-red-350 text-xs">
            ...
            {page === 'transfer' && (
              ...
              >
                <div className="luxury-card p-8 rounded-3xl flex flex-col gap-6">
                  <h3 className="font-bold text-lg text-center text-white">Lock Escrow Funds</h3>
                  <p className="text-xs text-stone-500 text-center">Submit a signed transaction including your arbitration parameters.</p>

                  <div className="flex flex-col gap-4">
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-stone-400">Recipient / Arbitrator Address</label>
                      <input 
                        value={destination} 
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-stone-400">Lock Amount (XLM)</label>
                      <input 
                        type="number"
                        value={amount} 
                    ...
                    <div className="flex flex-col gap-1.5">
                      <label className="text-xs font-bold text-stone-400">Arbitration Memo</label>
                      <input 
                        value={memo} 
                  ...
                  >
                    {txState === 'pending' ? 'Locking Funds...' : 'Initialize Escrow Lock'}
                  </button>
                    ...
                    <div className="flex flex-col gap-2 mt-2">
                      ...
                      {selectedWallet === 'freighter' ? (
                        ...
                        <div className="font-mono text-xs p-4 rounded-xl bg-stone-950 border border-stone-800 text-amber-500 text-center block break-all">
                          {txHash} (Simulated EVM Escrow Synced)
                        </div>
                      )}
                    </div>
                  )}
            ...
            {page === 'contract' && (
              ...
              >
                <div className="luxury-card p-8 rounded-3xl flex flex-col gap-6">
                  <h3 className="font-bold text-lg text-center text-white">Escrow Smart Contract</h3>
                  ...
                  <button 
                    ...
                    disabled={txState === 'pending'}
                    className="w-full py-4 rounded-xl bg-amber-700 hover:bg-amber-800 font-bold text-white shadow-lg shadow-amber-800/15 transition-all duration-300"
                  >
                    {txState === 'pending' ? 'Invoking Contract...' : 'Invoke Escrow Smart Contract'}
                  </button>
              ...
              >
                <div className="luxury-card p-8 rounded-3xl flex flex-col gap-6">
                  <div className="text-center flex flex-col gap-2">
                    <h3 className="font-bold text-lg text-white">Escrow Event Log</h3>
