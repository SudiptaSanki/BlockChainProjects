# 🚀 BlockFund: Decentralized Crowdfunding Vaults

BlockFund: Decentralized Crowdfunding Vaults is a premium decentralized Web3 application built on the Stellar network and Soroban smart contracts. It provides trustless milestone crowdfunding with trustless, cryptographic settlements.
...
- `level-1-white-belt/`:
  ...
  - `contracts/crowdfund_vault/`: Soroban Rust smart contract source code (`Cargo.toml`, `src/lib.rs`).
- `level-2-yellow-belt/`:
  - `contracts/crowdfund_vault/`: Soroban Rust smart contracts managing contract logic (`Cargo.toml`, `src/lib.rs`).
...
- `crowdfund_vault/`: Top-level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
- `contracts/crowdfund_vault/`: Root level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
...
```mermaid
graph TD
    A[Backer / Contributor] -->|Select Campaign Pledge| B(BlockFund Interface)
    B -->|Sign Pledge Transaction| C{Crowdfund Vault Contract}
    C -->|Lock XLM Escrow| D[Campaign Goal Milestone Check]
    D -->|Goal Reached| E[Disburse Funds to Builder]
    E -->|Horizon Event Synced| F[Real-Time Ledger Update]
```
...
- **Soroban Smart Contract:** Complete Rust smart contract package located at `level-1-white-belt/contracts/crowdfund_vault/` (`Cargo.toml`, `src/lib.rs`).
...
![Level 1 Submission Screenshot](./screenshots/level1_blockfund.png)
...
- **Soroban Contracts:** Integration with Rust smart contracts deployed on the Stellar Testnet located in `level-2-yellow-belt/contracts/crowdfund_vault/` (`Cargo.toml`, `src/lib.rs`).
...
Transaction Hash: b2c3d4e5f6a7890123456789abcdef0123456789abcdef0123456789abcdef01
...
- **Testnet Transaction Hash:** [Stellar Expert - Transaction b2c3d4e5...](https://stellar.expert/explorer/testnet/tx/b2c3d4e5f6a7890123456789abcdef0123456789abcdef0123456789abcdef01)

### 📸 Submission Screenshots

#### Available Wallet Options & Contract Invocations
![Level 2 Available Wallets](./screenshots/level2_blockfund.png)
