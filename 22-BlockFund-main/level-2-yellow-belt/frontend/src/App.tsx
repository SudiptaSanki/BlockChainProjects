...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 flex font-sans">
      ...
      {/* Light Sidebar */}
      <aside className="w-72 bg-white border-r border-slate-200 p-6 flex flex-col justify-between shrink-0 shadow-sm">
        ...
        <div className="max-w-4xl mx-auto flex flex-col gap-8">
          ...
          {page === 'wallets' && (
            <div className="flex flex-col gap-4">
              <h3 className="text-sm font-bold text-slate-700 uppercase tracking-wider">Select Wallet Gateway</h3>
              <div className="grid sm:grid-cols-2 gap-4">
                {walletOptions.map((w) => (
                  <button
                    key={w.id}
                    onClick={() => connectWallet(w.id)}
                    className="p-5 rounded-2xl bg-white border border-slate-200 hover:border-sky-500 transition-all flex items-center gap-4 text-left shadow-sm"
                  >
              ...
              <div className="mt-4 p-4 rounded-xl bg-slate-100 border border-slate-200 flex flex-col gap-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => simulateError('WalletNotFound')} className="px-3 py-1.5 rounded-lg bg-white text-xs text-slate-700 border border-slate-300">WalletNotFound</button>
                  <button onClick={() => simulateError('WalletConnectionRejected')} className="px-3 py-1.5 rounded-lg bg-white text-xs text-slate-700 border border-slate-300">WalletConnectionRejected</button>
                  <button onClick={() => simulateError('InsufficientBalance')} className="px-3 py-1.5 rounded-lg bg-white text-xs text-slate-700 border border-slate-300">InsufficientBalance</button>
                </div>
              </div>
            </div>
          )}
