# 🚀 PulsePoll: On-Chain Live Voting Protocol

PulsePoll: On-Chain Live Voting Protocol is a premium decentralized Web3 application built on the Stellar network and Soroban smart contracts. It provides trustless community governance & voting with trustless, cryptographic settlements.
...
- `level-1-white-belt/`:
  ...
  - `contracts/vote_poll/`: Soroban Rust smart contract source code (`Cargo.toml`, `src/lib.rs`).
- `level-2-yellow-belt/`:
  - `contracts/vote_poll/`: Soroban Rust smart contracts managing contract logic (`Cargo.toml`, `src/lib.rs`).
...
- `vote_poll/`: Top-level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
- `contracts/vote_poll/`: Root level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
...
```mermaid
graph TD
    A[Voter / Token Holder] -->|Select Ballot Proposal| B(PulsePoll Portal)
    B -->|Sign Ballot Transaction| C{Vote Poll Contract}
    C -->|Verify Token Weight| D[Record Immutable Vote]
    D -->|Aggregate Results| E[Stellar Testnet Ledger]
    E -->|Real-Time Sync| F[Poll Ticker Updated]
```
...
- **Soroban Smart Contract:** Complete Rust smart contract package located at `level-1-white-belt/contracts/vote_poll/` (`Cargo.toml`, `src/lib.rs`).
...
![Level 1 Submission Screenshot](./screenshots/level1_pulsepoll.png)
...
- **Soroban Contracts:** Integration with Rust smart contracts deployed on the Stellar Testnet located in `level-2-yellow-belt/contracts/vote_poll/` (`Cargo.toml`, `src/lib.rs`).
...
Transaction Hash: e5f6a7b8c9d0123456789abcdef0123456789abcdef0123456789abcdef01234
...
- **Testnet Transaction Hash:** [Stellar Expert - Transaction e5f6a7b8...](https://stellar.expert/explorer/testnet/tx/e5f6a7b8c9d0123456789abcdef0123456789abcdef0123456789abcdef01234)

### 📸 Submission Screenshots

#### Available Wallet Options & Contract Invocations
![Level 2 Available Wallets](./screenshots/level2_pulsepoll.png)
