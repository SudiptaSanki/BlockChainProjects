...
export async function fetchXlmBalance(publicKey: string): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(response.status === 404 ? 'Account not funded. Run Friendbot activation.' : 'Could not query balance.');
  }
...
export async function submitPayment(publicKey: string, destination: string, amount: string, memo: string): Promise<string> {
  ...
  const signedResult = await signFn(xdr, {
    networkPassphrase: StellarSdk.Networks.TESTNET,
    network: 'TESTNET',
    accountToSign: publicKey,
  });

  const signedXdr = typeof signedResult === 'string' ? signedResult : signedResult?.signedTxXdr || signedResult?.signedXDR || signedResult?.result;
  if (!signedXdr) {
