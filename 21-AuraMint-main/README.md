# 🚀 AuraMint: Decentralized NFT Minter

AuraMint: Decentralized NFT Minter is a premium decentralized Web3 application built on the Stellar network and Soroban smart contracts. It provides on-chain nft & asset minting vaults with trustless, cryptographic settlements.
...
- `level-1-white-belt/`:
  ...
  - `contracts/nft_minter/`: Soroban Rust smart contract source code (`Cargo.toml`, `src/lib.rs`).
- `level-2-yellow-belt/`:
  - `contracts/nft_minter/`: Soroban Rust smart contracts managing contract logic (`Cargo.toml`, `src/lib.rs`).
...
- `nft_minter/`: Top-level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
- `contracts/nft_minter/`: Root level Soroban Rust smart contract package (`Cargo.toml`, `src/lib.rs`).
...
```mermaid
graph TD
    A[Creator / Artist] -->|Upload Asset Metadata| B(AuraMint Portal)
    B -->|Request Freighter Signature| C{Soroban NFT Contract}
    C -->|Verify Mint Cap| D[Mint Unique Token ID]
    D -->|Register Ownership| E[Stellar Testnet Ledger]
    E -->|Horizon Event Synced| F[NFT Token Live in Wallet]
```
...
- **Soroban Smart Contract:** Complete Rust smart contract package located at `level-1-white-belt/contracts/nft_minter/` (`Cargo.toml`, `src/lib.rs`).
...
![Level 1 Submission Screenshot](./screenshots/level1_auramint.png)
...
- **Soroban Contracts:** Integration with Rust smart contracts deployed on the Stellar Testnet located in `level-2-yellow-belt/contracts/nft_minter/` (`Cargo.toml`, `src/lib.rs`).
...
- **Testnet Transaction Hash:** [Stellar Expert - Transaction a1b2c3d4...](https://stellar.expert/explorer/testnet/tx/a1b2c3d4e5f67890123456789abcdef0123456789abcdef0123456789abcdef0)

### 📸 Submission Screenshots

#### Available Wallet Options & Contract Invocations
![Level 2 Available Wallets](./screenshots/level2_auramint.png)

#### Deployed Smart Contract Called & Action Executed
![Level 2 Contract Call](./screenshots/level2_transaction_auramint.png)

#### Mobile Responsive UI
![Level 2 Mobile UI](./screenshots/level2_mobile_auramint.png)
